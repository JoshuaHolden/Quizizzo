...
AniMates—animation with your mates—is a server-authoritative drawing and guessing game built on the reusable drawing framework. Every participant first receives a different private absurd prompt for a three-frame animation, then creates a five-frame animation from one shared prompt in the Same Prompt Showdown. The display sees completion activity but not prompts or live work, then presents the saved animations anonymously.

## State machine

Round 1 uses `Briefing`, `Drawing`, `Guessing`, `Choosing`, and `Results`. The presenter briefing carries semantic copy so voice can replace or accompany it later, and uses a distinct stage background with a generic accessible drawing tutorial covering frame navigation, onion skin, undo, eraser, preview, and submission. Drawing happens once for everyone; each submitted animation then receives its own guessing, answer-choice, and result cycle. Briefings, drawing, guessing, choosing, reveals, and results use server-owned UTC deadlines and progress automatically; the host can use a clearly labelled skip control to continue early. Completing all required player actions also advances eligible phases early. Deadline commands travel through the same per-game command channel as all other mutations.
...
Game state contains opaque asset IDs, never physical paths. The display presentation receives only the current animation: Phaser loads local asset URLs and cycles frames at the server-described 300 ms per frame. During choosing and reveal, the animation occupies a taped paper card on the left while every high-contrast lettered answer sits inside a separate rounded game board on the right; the compact player rail remains below both regions. Results identify the correct answer and guess writers without reflowing or overlapping the animation.
...
After the final Round 1 reveal, the game enters a second presenter briefing and then gives every player the same randomly selected catalogue prompt with a five-frame drawing controller. All five-frame submissions remain anonymous while the TV plays each animation for three complete loops. Voting opens automatically after playback, and the host may continue early through the owner-only skip control. Phone vote options contain animation previews labelled A/B/C and exclude the player's own submission; forged self-votes are rejected by the module.
