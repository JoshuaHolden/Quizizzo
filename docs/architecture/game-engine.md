...
The deployed MVP intentionally runs one active Web process. PostgreSQL snapshots make restart recovery durable and the per-game actors efficiently serialize many concurrent parties, while Redis distributes SignalR refresh hints. Actor ownership and presence are still process-local. Running multiple Web replicas without distributed command and presence ownership would create competing actors even though the SignalR backplane exists, so it is explicitly unsupported. Scale this version vertically; horizontal Web scaling requires a later coordinated design and sticky sessions rather than a replica-count change.
