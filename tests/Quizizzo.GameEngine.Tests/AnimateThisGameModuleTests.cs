...
public sealed class AnimateThisGameModuleTests
{
    ...
    public void Missing_frames_copy_the_latest_completed_frame_and_navigation_state_survives_views()
    {
        ...
        Assert.Equal(AnimateThisGameModule.VotingPhase, fixture.State.Phase);
        Assert.Equal([firstFrame, firstFrame, firstFrame], firstAnimation.FrameAssetIds);
        Assert.Equal("animate-this:vote", configuration.SelectionScope);
    }
    ...
    public void Self_voting_is_forbidden_and_results_score_popularity()
    {
        ...
        Assert.Equal(AnimateThisGameModule.ResultsPhase, fixture.State.Phase);
    ...
    {
        var module = new AnimateThisGameModule();
    ...
    public async Task Runtime_makes_animation_submission_idempotent()
    {
        ...
        var module = new AnimateThisGameModule(TimeSpan.FromMinutes(2), TimeSpan.FromMinutes(1));
    ...
    {
        var clock = new AdjustableTimeProvider(Now);
        var module = new AnimateThisGameModule(TimeSpan.FromMinutes(1), TimeSpan.FromMinutes(1));
        await using var manager = new GameRuntimeManager(
            new GameModuleCatalog([module]), new InMemoryGameStateStore(), clock);
        var instanceId = GameInstanceId.New();
        var partyId = Guid.NewGuid();
    ...
    {
        private readonly AnimateThisGameModule module = new(TimeSpan.FromMinutes(2), TimeSpan.FromMinutes(1));
