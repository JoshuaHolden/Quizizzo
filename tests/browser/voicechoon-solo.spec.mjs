...
test("one human supplies a bot-backed Greensleeves band without refresh bursts", async ({ browser }, testInfo) => {
    ...
    await expect(player.locator(".voice-recording-card").first()).toBeVisible({ timeout: 30000 });
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        await expect(current).toHaveClass(/recorded/);
    }
    await player.getByRole("button", { name: "Send my sound pack" }).click();
    await expect(player.getByText("GET READY")).toBeVisible({ timeout: 20000 });
    ...
    // The bot-backed Greensleeves band schedules three charts and real chord clusters together.
    ...
    expect(largestBurst).toBeLessThanOrEqual(40);
